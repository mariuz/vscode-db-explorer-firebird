...
- [ ] Loading progress while the Schema Designer assembles its diagram — mirroring mssql 1.43.0's "detailed loading progress for Table Designer and Schema Designer". Checked before listing: there is no `withProgress` anywhere in `src/schema-designer/`, and opening the designer runs `getSchemaColumnsQuery()` plus `getForeignKeysQuery()` over the whole database before the webview draws anything, so a large schema is an unexplained pause with no indication it is working or how far along it is
...
- [ ] **A failing statement should say which line it is on.** mssql 1.43.0 lists "line numbers in query error messages may not be aligned" as a fix; here they are not merely misaligned, they are absent. `BatchResult` carries `sql`, `message`, `error` and `durationMs` — no position — so a script that fails on statement 40 of 60 reports the server's message with nothing tying it to a line, and the only way to find it is to count statements by eye. The infrastructure already exists and is not wired up: `splitStatementsWithOffsets()` returns each statement's `[start, end)` range and is already used by Run Statement Under Cursor and the Document Symbol provider, while `Driver.runBatch()` calls the text-only `splitStatements()`. Carrying the range into `BatchResult` would let the results view link an error back to its statement, and would make the error available as a real `Diagnostic` in the editor rather than only as text in a webview
- [ ] **A Messages pane, separate from the results grid** — mirroring `mssql.messagesDefaultOpen` and 1.44.0's "option to include timestamps when using Copy All in the Messages panel". Today a statement's outcome message lives inside that statement's own result tab, so reading what a 60-statement batch actually did means clicking through 60 tabs, and there is no way to copy the lot. Verified absent rather than assumed: nothing in `src/result-view/` mentions a messages tab. The 1.44.0 timestamp option is the useful detail — a message log without times cannot tell you which statement was slow
...
- [ ] Choose where query results open — mssql has `openQueryResultsInTabByDefault` and `defaultQueryResultsViewMode` because it can put results in the bottom panel or in an editor tab. `queryResultsView.ts` hardcodes `ViewColumn.Two`, so results always take an editor group; there is no way to dock them to the panel and keep the editor full-width. Smaller than it sounds, and worth doing alongside the Messages pane above rather than on its own
