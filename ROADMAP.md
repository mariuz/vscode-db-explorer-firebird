...
- [x] **Declare and test Workspace Trust** — now `{ supported: false, description }`, which changes no behaviour (a missing declaration already means "disabled in Restricted Mode") but replaces silence with a stated reason. It is the honest answer, not a cautious one: `firebird.isqlPath`/`gbakPath`/`dockerPath` are window-scoped, so a repository's own `.vscode/settings.json` can point them at any executable. **Now `{ supported: "limited" }`**: the extension loads in an untrusted folder, with the folder's own executable-path settings ignored via `restrictedConfigurations` (lighter than machine scope, which would forbid a legitimate per-project `isql` even in a trusted folder) and its `.vscode/firebird.json` connections withheld — that file cannot carry a password, so a malicious entry would prompt for one. Verified in a real Restricted Mode window. Verified by manifest tests (`src/test/manifest.test.ts`) rather than an untrusted-mode test config, which is not achievable for a `supported: false` extension: it does not load in Restricted Mode, so nothing would be left running to assert from
