...
- [ ] **SQL schemas** — a schema level in the Object Explorer, `RDB$SCHEMA_NAME` in every metadata query, schema-qualified DDL/scripting/row-editing, `SET SEARCH_PATH`-aware completion and a "New Query in Schema" action, and color-coded schemas in the Schema Designer. **Phases 1a–1b done**: engine-version detection (cached per connection), a schema-aware Tables listing with qualified labels and SQL, and schema-filtered column metadata — all verified against a live Firebird 6.0.0 server with a deliberate `SALES.ORDERS`/`PUBLIC.ORDERS` collision, where the old code demonstrably queried the wrong table and listed a merged set of columns belonging to no real table. Still missing: the other object categories and the Schemas tree level ([design doc](docs/roadmap/firebird6-schemas.md))
