...
- [ ] **SQL schemas** — a schema level in the Object Explorer, `RDB$SCHEMA_NAME` in every metadata query, schema-qualified DDL/scripting/row-editing, `SET SEARCH_PATH`-aware completion and a "New Query in Schema" action, and color-coded schemas in the Schema Designer. **Phase 1a done**: engine-version detection (cached per connection), a schema-aware Tables listing, and qualified labels and SQL for tables — verified against a live Firebird 6.0.0 server with a deliberate `SALES.ORDERS`/`PUBLIC.ORDERS` collision, where the old unqualified SQL demonstrably hit the wrong table. Still missing: the other object categories, schema-filtered column metadata, and the Schemas tree level ([design doc](docs/roadmap/firebird6-schemas.md))
