...
- **Running an empty editor reported a crash.** With nothing written yet — or a file holding only comments — **Run Firebird Query** answered *"Oops! Something went wrong. Check the log output for more details!"* and offered a log with nothing in it. It now says plainly what is wrong, and distinguishes the cases: an empty file, an empty selection, a file containing only comments, or one containing only semicolons.

- **The Backup & Restore Wizard's backups always failed.** Its **Include Statistics** option — which is ticked by default — sent `gbak` a bare `-st`, but that switch requires a value saying which counters to report. gbak read the next option as that value and stopped with `wrong char "-" at statistics parameter` without writing a backup. It now sends `-st TDRW` (time, delta, reads, writes) and the backup completes.
- **The Backup & Restore Wizard's "Deactivate Triggers" option always failed the restore.** It sent `-inhibit_triggers`, which is not a `gbak` switch; gbak answered `unknown switch "INHIBIT_TRIGGERS"` and created no database. gbak has no equivalent option on restore, so the checkbox has been removed rather than silently changed to mean something else.
- **The Backup & Restore Wizard ignored the `firebird.gbakPath` setting.** It always ran whatever `gbak` happened to be on your `PATH`, so on a machine where Firebird's tools are installed somewhere else the wizard could not work at all, while **Backup Database** and **Restore Database** — which do read the setting — worked fine. All three now resolve gbak the same way.
- **Object Properties showed invalid DDL for any column with a default.** The **DDL Source** tab, and the **Copy DDL** button beside it, wrote `DEFAULT DEFAULT 0` for a column declared `DEFAULT 0` — Firebird stores the keyword as part of the value and the panel added a second one. Pasting that DDL back into a script would fail.
- **Object Properties could render an object name or an error message as markup.** The loading and error panels put both straight into the page without escaping, so a table created with a quoted name containing `<`, or a server message quoting your SQL, displayed incorrectly. Both are escaped now, as the rest of the panel already was.
...
- **A connection's default schema meant two different things depending on the driver (Firebird 6).** With **Set Default Schema…** set to `SALES`, the pure-JS driver left `PUBLIC` reachable behind it, while the native driver (`firebird.useNativeDriver`) did not — so turning that setting on, which is meant to be a choice about wire encryption, made every unqualified reference to a table in `PUBLIC` fail with *Table unknown*. Both drivers now put the same search path in place.
- **A result tab's row count was invisible whenever its statement was long.** Each tab in a batch run carries a badge — the number of rows, a tick, or a warning sign — but it was laid out as plain text inside a fixed-width tab that clips what does not fit, so any statement wide enough to be truncated pushed its own badge out of sight. The statement text is now what truncates, and the badge always shows.
...
- **A connection's default schema now applies from the moment it attaches, on the native driver too (Firebird 6).** With `firebird.useNativeDriver` turned on, the default schema was put in place by sending a `SET SEARCH_PATH` statement straight after connecting — an extra round trip on every connection, and a short window in which anything that ran first still saw the server's own default. Firebird Studio asked the driver's authors for a way to set it as part of the connection itself; they added one, and it is what Firebird Studio uses now. Both drivers put the schema in place before the session's first statement.
- **The Schema Designer says what it is doing while it loads.** Opening it on a large database showed a motionless *Loading schema…* and nothing else, so a slow catalogue read and a stuck one looked identical. It now names each stage as it happens — checking the server version, reading tables, columns and relationships, then laying out the diagram with the table count it found — next to a spinner that shows it is still working.
...
- **Give a connection a default schema (Firebird 6).** Right-click a database for **Set Default Schema…**, pick one of the server's schemas, and unqualified names in that connection resolve through it from then on — in the tree, in autocomplete and in every query, not just in one document the way **New Query in Schema…** works. `PUBLIC` stays behind it as a fallback. A project can declare it too, as `"defaultSchema"` in `.vscode/firebird.json`. Servers older than 6 have no schemas, and the command says so rather than storing a setting that would do nothing.
...
- **Autocompletion no longer re-reads the whole schema on every keystroke.** Each completion (and each hover) opened a new database connection and re-ran two catalogue queries — one of which names every table in the database — and never closed the connection afterwards. On a large database that made typing progressively slower and left a growing number of open attachments on the server. The schema is now cached for 30 seconds, and refreshing the connection tree or switching connections picks up changes immediately. Measured against a real server: the second and third completions went from 29 ms and three open connections to under 1 ms and one.
- **Autocompletion now gives up waiting rather than hanging.** If the schema takes too long, Firebird Studio offers its keyword list straight away instead of nothing, and the table and column names appear as soon as the schema arrives. `firebird.intelliSense.completionTimeoutMs` sets how long it waits (3 seconds by default; `0` waits indefinitely, which is what previous versions did).

- **Ctrl+F now searches inside Firebird Studio's panels.** Query results, the query plan, the Schema Designer, the profiler, Object Properties and the Backup & Restore wizard's output log all support VS Code's find widget. Two things to know: it searches what the panel is currently showing, so with a paged result that is the page on screen — the grid's own filter is what covers every row — and it is not available when results are docked in the bottom Panel (`firebird.queryResultsLocation: "panel"`), which VS Code does not offer it for.

- **Move a Database Project object into another schema.** Right-click a project's `.sql` file in the Explorer — or use **Firebird: Move to Schema...** with it open — to move it into another schema's folder and requalify its `CREATE` statement. Pick an existing schema, type a new one, or move it back to the default schema, which restores the plain `tables/ORDERS.sql` layout rather than leaving a `schemas/` folder behind. The project's manifest is updated in place, so **Build** still concatenates everything in its existing dependency-safe order. Two things it deliberately does not do: it changes the project, not a database — publish the project to apply it — and it rewrites only the object's own `CREATE` statement, so if other files still refer to the object by name (the foreign-key script, a trigger, a procedure body) it tells you which ones rather than editing them for you. Roles and users cannot be moved, because Firebird has no schema for them.

- **`Shift+Alt+F` now formats SQL.** Firebird Studio has had a SQL formatter since early on, but it was only reachable from the **Format SQL** command, so VS Code's own formatting — `Shift+Alt+F`, **Format Selection**, `editor.formatOnSave`, `editor.formatOnType` — did nothing in a `.sql` file, and formatting a notebook cell was not possible at all. All of them work now. Formatting follows the indentation your editor is already using, rather than a separate setting that could disagree with it, and a file that is already formatted produces no edit — so turning on format-on-save will not mark every SQL file as modified.
- **Choose how formatting cases keywords.** The new `firebird.format.keywordCase` setting takes `upper` (the default, and what the formatter has always done), `lower`, or `preserve` to leave your own casing alone and only change the layout.

- **Query results can now dock in the bottom Panel, next to Terminal and Problems.** They have always taken an editor group, so running a query halved the width of the file you were reading it against. Set `firebird.queryResultsLocation` to `panel` and results open in the Panel instead, leaving the editor full width; the default is unchanged, so nothing moves unless you ask. The choice applies to the next query you run — a results view already open stays where it is.

- **A Messages tab, showing what a whole run did in one place.** A statement's outcome used to live inside that statement's own result tab, so reading what a 60-statement script actually did meant clicking through 60 tabs, and there was no way to copy the lot. Query results now carry a **Messages** tab listing every statement in order — when it ran, which line it is on, what it returned or what it failed with, and how long it took — under a summary that says how many statements ran, how many failed and how long the batch took altogether. **Copy All** puts the whole log on the clipboard; clicking a line goes to that statement in the editor, and clicking anywhere else on the row opens that statement's own results. Set `firebird.messagesDefaultOpen` to land on it instead of the first result, and `firebird.messagesIncludeTimestamps` to `false` to leave the times out of what you copy.

- **A statement that fails now says which line it is on.** Running a script and having statement 40 of 60 fail reported the server's message — *Token unknown* — with nothing tying it to the file, so finding it meant counting semicolons by eye. Each result now carries the position of the statement it came from: the failed tab is badged with its line (*⚠ 40*), the error itself is headed **Line 40, column 8**, and clicking that heading jumps the editor to it. Where Firebird names a position of its own it is used, mapped from the statement it counts inside onto the file you are looking at — so an error on the third line of a `CREATE PROCEDURE` points at that line, not at the statement's first. Failures also appear as ordinary red squiggles in the editor, under Problems, and are cleared by your first edit. Running a selection or a single statement is handled the same way, counting from where it actually sits in the file rather than from line 1. A notebook cell's errors say where in the cell they happened too.

- **Filter and sort a large table from the grid, across all of it.** When you run `SELECT * FROM SOMETABLE` and it is bigger than the row limit, the grid gains a filter — pick a column, a comparison (contains, starts with, =, ≠, >, <, is null) and a value — and clicking a column header sorts. Both are sent to the server as a new query, so they cover every row in the table rather than the page you can see: sorting a 25 000-row table by id descending shows you row 25 000, not the largest of the rows already on screen. Offered only for a query that selects a whole table, because filtering anything else would mean rewriting the query you wrote.
